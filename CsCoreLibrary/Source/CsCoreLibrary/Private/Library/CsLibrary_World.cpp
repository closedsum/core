...
namespace NCsWorld
{
	namespace NLibrary
	{
		namespace NCached
		{
			namespace Str
			{
				...
				CS_DEFINE_CACHED_FUNCTION_NAME_AS_STRING(NCsWorld::FLibrary, GetSafeAsObject);
			}
		}
	}
	...
	FString FLibrary::PrintActorAndClass(const AActor* Object)
	{
		...
		return FString::Printf(TEXT("Actor: %s with Class: %s"), *(Object->GetName()), *(Object->GetClass()->GetName()));
	}
