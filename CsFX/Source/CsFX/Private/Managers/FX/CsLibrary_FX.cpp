...
#include "Niagara/Private/NiagaraDataInterfaceArrayImplHelpers.h"
#include "NiagaraFunctionLibrary.h"
...
namespace NCsFX
{
	namespace NLibrary
	{
		namespace NCached
		{
			namespace Str
			{
				CS_DEFINE_CACHED_FUNCTION_NAME_AS_STRING(NCsFX::NLibrary::FLibrary, SafeHasVariableName);
				CS_DEFINE_CACHED_FUNCTION_NAME_AS_STRING(NCsFX::NLibrary::FLibrary, SafeHasParameter);
				CS_DEFINE_CACHED_FUNCTION_NAME_AS_STRING(NCsFX::NLibrary::FLibrary, Spawn_Internal);
			}

			namespace Name
			{
				CS_DEFINE_CACHED_FUNCTION_NAME_AS_NAME(NCsFX::NLibrary::FLibrary, Spawn_Internal);
			}
		}
	}

	CS_DEFINE_STATIC_LOG_LEVEL(FLibrary, NCsFX::FLog::Warning);
	...
	using ParamsManagerType = NCsFX::NSpawn::NParams::FManager;
	using ParamsResourceType = NCsFX::NSpawn::NParams::FResource;
	using ParamsType = NCsFX::NSpawn::NParams::FParams;

	using PayloadType = NCsFX::NLibrary::NSetArrayFloatChecked::FPayload;
	...
	UNiagaraSystem* FLibrary::SafeLoad(const FString& Context, const FSoftObjectPath& Path, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
		return CsObjectLibrary::SafeLoad<UNiagaraSystem>(Context, Path, Log);
	}

	UNiagaraSystem* FLibrary::SafeLoad(const FString& Context, const FString& Path, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
		return CsObjectLibrary::SafeLoad<UNiagaraSystem>(Context, Path, Log);
	}
	...
	bool FLibrary::IsValidUserVariableName(const FString& Context, const FName& Name, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
	...
	bool FLibrary::SafeHasVariableName(const FString& Context, UNiagaraSystem* System, const FName& Name, const ParameterValueType& ValueType, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
		// Check System is Valid
	...
	bool FLibrary::SafeHasParameter(const FString& Context, UNiagaraSystem* System, const ParameterType* Parameter, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
		// Check System is Valid
	...
	UNiagaraDataInterfaceSkeletalMesh* FLibrary::GetSafeDataInterface(const FString& Context, UNiagaraComponent* Component, const SkeletalMeshParameterType* Parameter, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
	...
	void FLibrary::SetSafeParameter(const FString& Context, UNiagaraComponent* Component, const SkeletalMeshParameterType* Parameter, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
	...
	*/
	...
	FNiagaraVariable* FLibrary::GetSafeExposedVariable(const FString& Context, UNiagaraSystem* System, const FName& Name, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
	...
	bool FLibrary::SafeIsExposedVariableInt(const FString& Context, UNiagaraSystem* System, const FName& Name, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
	...
	bool FLibrary::SetSafeExposedVariableInt(const FString& Context, UNiagaraSystem* System, const FName& Name, const int32& Value, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
		CS_IS_PTR_NULL(System)
		CS_IS_NAME_NONE(Name)
	...
	#if WITH_EDITOR
	bool FLibrary::SafeExposedParameters_PostGenericEditChange(const FString& Context, UNiagaraSystem* System, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
		CS_IS_PENDING_KILL(System)
	...
	FCsRoutineHandle FLibrary::SpawnChecked(const FString& Context, UObject* WorldContext, ParamsResourceType* Params)
	{
		...
		Payload->CoroutineImpl.BindStatic(&CsFXLibrary::COROUTINE);
	...
	FCsRoutineHandle FLibrary::SafeSpawn(const FString& Context, UObject* WorldContext, ParamsResourceType* Params, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
	...
	FCsRoutineHandle FLibrary::SafeSpawn_GameState_OnceWithDelay(const FString& Context, UObject* WorldContext, const FCsFX& FX, const float& Delay, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
	...
	FNiagaraSystemInstance* FLibrary::GetSafeSystemInstance(const FString& Context, UNiagaraComponent* Component, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
		CS_IS_PENDING_KILL_RET_NULL(Component)
	...
	bool FLibrary::SafeHasSystemInstance(const FString& Context, const UNiagaraComponent* Component, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
	...
	TSharedPtr<FNiagaraSystemSimulation, ESPMode::ThreadSafe> FLibrary::GetSafeSystemSimulation(const FString& Context, UNiagaraComponent* Component, CS_FN_PARAM_DEFAULT_LOG_LEVEL_COMMENT)
	{
