...
namespace NCsGraphPinEnumStructMaskCached
{
	namespace Str
	{
		extern CSENUMEDITOR_API const FString INVALID;;
	}

	namespace Txt
	{
		extern CSENUMEDITOR_API const FText INVALID;
	}
}

#pragma endregion Cache

class CSENUMEDITOR_API SCsGraphPin_EnumStructMask : public SGraphPin
{
public:
	SLATE_BEGIN_ARGS(SCsGraphPin_EnumStructMask) {}
	SLATE_END_ARGS()
...
protected:
	...
	void Construct_Internal()
	{
		...
		{
			if (!EnumMap::Get().IsEmpty())
			{
				const EnumStruct& Enum = EnumMap::Get().GetEnumAt(0);
	...
	*/
	...
	void ComboBoxSelectionChanged_Internal(TSharedPtr<int32> NewSelection, const ESelectInfo::Type& SelectInfo)
	{
		...
		{
			EnumSelectionString = TEXT("(Value=0,Mask=0,Name_Internal=\"None\")");;
		}
