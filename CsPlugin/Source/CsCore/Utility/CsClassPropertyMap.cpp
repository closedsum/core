...
bool FCsClassPropertyMap::IsValidMap(const FName& ClassName)
{
	...
	const TMap<FName, TWeakObjectPtr<UProperty>>& ClassMap = Map[ClassName];

	for (const TPair<FName, TWeakObjectPtr<UProperty>>& Pair : ClassMap)
	{
...
const TMap<FName, TWeakObjectPtr<UProperty>>& FCsClassPropertyMap::GetMap(const FName& ClassName)
{ 
...
{
	const TMap<FName, TWeakObjectPtr<UProperty>>& Map = FCsClassPropertyMap::Get().GetMap(ClassName);

	for (const TPair<FName, TWeakObjectPtr<UProperty>>& Pair : Map)
	{
		const FName& PropertyName   = Pair.Key;
		UProperty* Property			= Pair.Value.Get();
		...
		// int32
		if (UIntProperty* Int32Property = Cast<UIntProperty>(Property))
		{
...
void FCsClassInstancePropertyMap::CreateMaps_Byte(void* Object, UByteProperty* ByteProperty, const FName& PropertyName)
{
}

void FCsClassInstancePropertyMap::CreateMaps_Enum(void* Object, UEnumProperty* EnumProperty, const FName& PropertyName)
{
}

void FCsClassInstancePropertyMap::CreateMaps_Object(void* Object, UObjectProperty* ObjectProperty, const FName& PropertyName)
{
}
