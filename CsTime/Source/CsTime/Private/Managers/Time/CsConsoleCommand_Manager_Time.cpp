	...
	namespace NManager
	{
		namespace NConsoleCommand
		{
			namespace NCached
			{
				namespace Str
				{
					CSTIME_API const FString CategoryName = TEXT("Manager_Time");
		...
		bool FConsoleCommand::GetUpdateGroup(const FString& Context, const TCHAR*& Str, FECsUpdateGroup& OutValue, const FString& Definition)
		{
			...
			return ConsoleCommandLibrary::GetEnumStruct<EMCsUpdateGroup, FECsUpdateGroup>(Context, Str, OutValue, NConsoleCommand::NCached::Str::UpdateGroup, Definition, &NCsTime::FLog::Warning);
		}
		...
		bool FConsoleCommand::GetDeltaTime(const FString& Context, const TCHAR*& Str, float& OutValue, const FString& Definition)
		{
			...
			Params.Log		  = &NCsTime::FLog::Warning;
		...
		bool FConsoleCommand::Exec_Pause(const TCHAR* Cmd)
		{
			...
			void(*Log)(const FString&) = &NCsTime::FLog::Warning;
			...
			for (int32 I = 0; I < Count; ++I)
			{
				...
				if (OutString.RemoveFromStart(Command))
				{
					...
					{
						UE_LOG(LogCsTime, Warning, TEXT("%s: Command: %s."), *Context, *Definition);
						return false;
					}
