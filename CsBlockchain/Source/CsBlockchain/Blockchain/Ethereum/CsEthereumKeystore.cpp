...
void FCsEthereumKeystore::Parse(const FString &Str)
{
	...
	{
		UE_LOG(LogCsBlockchain, Warning, TEXT("FCsEthereumKeystore::Parse: Input string is NOT a Valid Genesis string."));
	}
}
...
void FCsEthereumKeystore::ParseFromFilePath(const FString &Path)
{
	...
	{
		UE_LOG(LogCsBlockchain, Warning, TEXT("FCsEthereumKeystore::ParseFromFilePath: Path %s does NOT Exist."), *Path);
	}
}