...
#ifndef __CS_MANAGER_RESOURCE_VALUE_TYPE_ABSTRACT_H__
#define __CS_MANAGER_RESOURCE_VALUE_TYPE_ABSTRACT_H__
...
class TCsManager_ResourceValueType_Abstract : public ICsManager_Resource
{
	static_assert(std::is_base_of<TCsResourceContainer<ResourceType>, ResourceContainerType>(), "TCsManager_ResourceValueType_Abstract: ResourceContainerType does NOT derive from TCsResourceContainer<ResourceType>.");
	
	static_assert(std::is_abstract<ResourceType>(), "TCsManager_ResourceValueType_Abstract: ResourceType must be Abstract.");

public:

	TCsManager_ResourceValueType_Abstract() :
		Name(),
		Name_Internal(),
		Resources(),
		Pool(),
		...
		AllocationOrder()
	{
		Name = TEXT("TCsManager_ResourceValueType_Abstract");
		Name_Internal = FName(*Name);

		AdvancePoolIndex = &TCsManager_ResourceValueType_Abstract<ResourceType, ResourceContainerType>::AdvancePoolIndexByOrder;
	}

	virtual ~TCsManager_ResourceValueType_Abstract()
	{
		Shutdown();
	}
...
private:
	...
	void (TCsManager_ResourceValueType_Abstract<ResourceType, ResourceContainerType>::* AdvancePoolIndex)();
...
protected:
	...
	virtual ResourceType* ConstructResourceType()
	{
		return nullptr;
	}
	...
	*/
	void CreatePool(const int32& InSize)
	{
		...
		for (int32 I = 0; I < PoolSize; ++I)
		{
			...
			M->SetIndex(I);
			ResourceType* R = ConstructResourceType();
			M->Set(R);
			Pool.Add(M);
			Resources.Add(R);
...
#endif // #ifndef __CS_MANAGER_RESOURCE_VALUE_TYPE_ABSTRACT_H__