...
struct CSDATA_API FCsDataRootSet
{
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CsData|Data", meta = (AllowedClasses = "/Script/Engine.World"))
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CsData|Data")
...
public:
	...
	uint8* GetSafeDataTableRow(const FString& Context, const EMember& MemberType, const FName& RowName, void(*Log)(const FString&) = &NCsData::FLog::Warning) const;

	uint8* GetSafeDataTableRow(const FString& Context, const EMember& MemberType, const UScriptStruct* RowStruct, const FName& RowName, void(*Log)(const FString&) = &NCsData::FLog::Warning) const;
	
	template<typename RowStructType>
	RowStructType* GetSafeDataTableRow(const FString& Context, const UObject* WorldContext, const EMember& MemberType, const FName& RowName, void(*Log)(const FString&) = &NCsData::FLog::Warning) const
	{
	...
	template<typename RowStructType>
	RowStructType* GetSafeDataTableRow(const FString& Context, const EMember& MemberType, const FName& RowName, void(*Log)(const FString&) = &NCsData::FLog::Warning) const
	{
		if (uint8* RowPtr = GetSafeDataTableRow(Context, MemberType, RowStructType::StaticStruct(), RowName, Log))
			return reinterpret_cast<RowStructType*>(RowPtr);
		return nullptr;
