...
/** @type {FunctionLibrary} */ 	var FunctionLibrary = NJsFunction.FLibrary;
...
module.exports = class FJsRoutine
{
	static FOnEnd = class OnEndClass
    {
        ...
        constructor()
        {
            ...
            /** @type {object} */	this.Caller = null;
        }

        Set(fn /*function*/, caller /*object*/)
        {
            let context = FJsRoutine.FOnEnd.NCached.NStr.Set;

			check(CommonLibrary.IsFunctionChecked(context, fn));

			// Check fn is of type: function
			check(FunctionLibrary.IsArgCountChecked(context, fn, 1));

			this.Fn = fn;
			this.Caller = caller;
        }

        Execute(r /*FJsRoutine*/)
        {
            if (CommonLibrary.IsValidObject(this.Caller))
                 return this.Fn.apply(this.Caller, [r]);
            return this.Fn.apply(null, [r]);
        }
    ...
    constructor()
    {
		...
		/** @type {boolean} */		this.bDeltaTimeScale = false;
		/** @type {FJsProperty} */  this.DeltaTimeScale = null;
	...
	AddOnEnd(fn /*function*/, /*caller*/)
	{
		let onEnd = new OnEndType();
		onEnd.Set(fn, caller);
	...
	*/
    End(endReason /*NJsCoroutine.EEndReason*/)
    {
		...
		{
			this.OnEnds[i].Execute(this.self);
        }
		this.OnEnds = [];
    }
