...
namespace NCsBTTask_LookAtType
{
	namespace Ref
	{
		CSCOREDEPRECATED_API CS_ADD_TO_ENUM_MAP_CUSTOM(EMCsBTTask_LookAtType, UntilAligned, "Until Aligned");
		CSCOREDEPRECATED_API CS_ADD_TO_ENUM_MAP(EMCsBTTask_LookAtType, Timed);
		CSCOREDEPRECATED_API CS_ADD_TO_ENUM_MAP(EMCsBTTask_LookAtType, Forever);
		CSCOREDEPRECATED_API CS_ADD_TO_ENUM_MAP_CUSTOM(EMCsBTTask_LookAtType, ECsBTTask_LookAtType_MAX, "MAX");
	}

	CSCOREDEPRECATED_API const uint8 MAX = (uint8)Type::ECsBTTask_LookAtType_MAX;
}
...
EBTNodeResult::Type UCsBTTask_LookAt::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (!Pawn)
	{
		UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_LookAt::ExecuteTask (%s.%s): This Task only works with Pawns derived from ACsAIPawn."), *(BasePawn->GetName()), *(BTree->GetName()));
		return EBTNodeResult::Failed;
	}
	...
	{
		UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_LookAt::ExecuteTask (%s.%s): InProgress."), *(Pawn->GetName()), *(BTree->GetName()));
	}
		...
		if (Type == ECsBTTask_LookAtType::UntilAligned)
		{
				...
				AbsDeltaYaw < Tolerance)
			{
				...
				if (CsCVarLogAIBTTasks->GetInt() == CS_CVAR_SHOW_LOG)
				{
					...
					UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_LookAt::TickTask (%s.%s): Succeeded."), *(Pawn->GetName()), *(BTree->GetName()));
				}
#endif // #if !UE_BUILD_SHIPPING
	...
	if (AAIController* AIController = OwnerComp.GetAIOwner())
	{
		if (ACsAIPawn* Pawn = Cast<ACsAIPawn>(AIController->GetPawn()))
		{
			...
			if (CsCVarLogAIBTTasks->GetInt() == CS_CVAR_SHOW_LOG)
			{
				...
				UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_LookAt::AbortTask (%s.%s): Aborted."), *(Pawn->GetName()), *(BTree->GetName()));
			}
#endif // #if !UE_BUILD_SHIPPING
		}
	}
