...
struct CSTYPES_API FCsCurve
{
	GENERATED_USTRUCT_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CsTypes|Curve")
	TSoftObjectPtr<UCurveBase> Curve;

	UPROPERTY(BlueprintReadOnly, Category = "CsTypes|Curve", meta = (Bitmask, BitmaskEnum = "/Script/CsTypes.ECsLoadFlags"))
	int32 Curve_LoadFlags;

	UPROPERTY(Transient, BlueprintReadOnly, Category = "CsTypes|Curve")
...
public:
	...
	* return			Curve
	*/
	UCurveBase* GetSafe(const FString& Context, void(*Log)(const FString&) = &NCsTypes::FLog::Warning) const
	{
...
USTRUCT(BlueprintType)
struct CSTYPES_API FCsCurveFloat
{
	GENERATED_USTRUCT_BODY()
...
public:
	...
	* return			Curve Float
	*/
	UCurveFloat* GetSafe(const FString& Context, void(*Log)(const FString&) = &NCsTypes::FLog::Warning) const
	{
		if (!Curve.ToSoftObjectPath().IsValid())
		{
			if (Log)
...
USTRUCT(BlueprintType)
struct CSTYPES_API FCsCurveVector
{
	GENERATED_USTRUCT_BODY()
...
public:
	...
	* return			Curve Vector
	*/
	UCurveVector* GetSafe(const FString& Context, void(*Log)(const FString&) = &NCsTypes::FLog::Warning) const
	{
		if (!Curve.ToSoftObjectPath().IsValid())
		{
			if (Log)
...
USTRUCT(BlueprintType)
struct CSTYPES_API FCsCurveLinearColor
{
	GENERATED_USTRUCT_BODY()
...
public:
	...
	* return			Curve Linear Color
	*/
	UCurveLinearColor* GetSafe(const FString& Context, void(*Log)(const FString&) = &NCsTypes::FLog::Warning) const
	{
		if (!Curve.ToSoftObjectPath().IsValid())
		{
			if (Log)
...
struct CSTYPES_API EMCsRichCurveInterpMode : public TCsEnumMap<ECsRichCurveInterpMode>
{
...
namespace NCsRichCurveInterpMode
{
	...
	namespace Ref
	{
		extern CSTYPES_API const Type RCIM_Linear;
		extern CSTYPES_API const Type RCIM_Constant;
		extern CSTYPES_API const Type RCIM_Cubic;
		extern CSTYPES_API const Type RCIM_None;
		extern CSTYPES_API const Type ECsRichCurveInterpMode_MAX;
	}
...
struct CSTYPES_API EMCsRichCurveTangentMode : public TCsEnumMap<ECsRichCurveTangentMode>
{
...
namespace NCsRichCurveTangentMode
{
	...
	namespace Ref
	{
		extern CSTYPES_API const Type RCTM_Auto;
		extern CSTYPES_API const Type RCTM_User;
		extern CSTYPES_API const Type RCTM_Break;
		extern CSTYPES_API const Type RCTM_None;
		extern CSTYPES_API const Type ECsRichCurveTangentMode_MAX;
	}
