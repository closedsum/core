...
void UCsButtonComponent_SkeletalMesh::RemoveInstigator(const TEnumAsByte<ECsButtonState::Type> &InState, UObject* InInstigator)
{
	...
	if (!Found)
	{
		UE_LOG(LogCs, Warning, TEXT("UCsButtonComponent_SkeletalMesh::RemoveInstigator(%s - %s): Failed to find Instigator: %s in %s List."), *GetName(), *((*InteractiveTypeToString)(Type)), *(InInstigator->GetName()), *ECsInteractiveState::ToString(InState));
	}
	*/
}
