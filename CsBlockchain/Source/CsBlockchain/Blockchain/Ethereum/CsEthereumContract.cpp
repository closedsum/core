...
void CsEthereumContract::Parse(const FString &Str)
{
	...
	{
		UE_LOG(LogCsBlockchain, Warning, TEXT("CsEthereumContract::Parse: Input string is NOT a Valid Genesis string."));
	}
}
...
void CsEthereumContract::ParseFromFilePath(const FString &Path)
{
	...
	{
		UE_LOG(LogCsBlockchain, Warning, TEXT("CsEthereumContract::ParseFromFilePath: Path %s does NOT Exist."), *Path);
	}
}
