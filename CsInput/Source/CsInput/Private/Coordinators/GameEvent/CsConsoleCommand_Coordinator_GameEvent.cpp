	...
	namespace NCoordinator
	{
		namespace NConsoleCommand
		{
			namespace NCached
			{
				namespace Str
				{
					CSINPUT_API const FString CategoryName = TEXT("Coordinator_GameEvent");
		...
		bool FConsoleCommand::GetGameEventCoordinatorGroup(const FString& Context, const TCHAR*& Str, FECsGameEventCoordinatorGroup& OutValue, const FString& Definition)
		{
			...
			return ConsoleCommandLibrary::GetEnumStruct<_EnumMapType, EnumType>(Context, Str, OutValue, EnumName, Definition, &NCsInput::FLog::Warning);
		}
		...
		bool FConsoleCommand::Exec_BroadcastGameEvent(const TCHAR* Cmd)
		{
			...
			for (int32 I = 0; I < Count; ++I)
			{
				...
				if (OutString.RemoveFromStart(Command))
				{
					...
					{
						UE_LOG(LogCsInput, Warning, TEXT("%s: Command: %s."), *Context, *Definition);
						return false;
					}
					...
					// Try Get Location
					{
						...
						{
							CsGameEventCoordinatorLibrary::SafeBroadcastGameEvent(Context, MyRoot, Group, GameEvent, 0.0f, Location, &NCsInput::FLog::Warning);
							return true;
						}
					}
					...
					// Try Get Value then Try Get Location
					{
						...
						if (GetValue(Context, StrAsChar, Value, Definition))
						{
							...
							if (ConsoleCommandLibrary::ConsumeNextCharAndCheckNotEmpty(Context, OutString, Definition))
							{
								...
								if (GetLocation(Context, StrAsChar, Location, Definition))
								{
									CsGameEventCoordinatorLibrary::SafeBroadcastGameEvent(Context, MyRoot, Group, GameEvent, Value, Location, &NCsInput::FLog::Warning);
									return true;
								}
							}
							CsGameEventCoordinatorLibrary::SafeBroadcastGameEvent(Context, MyRoot, Group, GameEvent, Value, FVector3f::ZeroVector, &NCsInput::FLog::Warning);
							return true;
						}
					}

					CsGameEventCoordinatorLibrary::SafeBroadcastGameEvent(Context, MyRoot, Group, GameEvent, 0.0f, FVector3f::ZeroVector, &NCsInput::FLog::Warning);
					return true;
				}
			}
			return false;
