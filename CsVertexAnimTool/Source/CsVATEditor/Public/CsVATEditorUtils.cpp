...
#include "CsVATEditorUtils.h"
#include "CsVATEditor.h"
...
#include "Editor/Persona/Private/PersonaToolkit.h"
			...
			static void SkinnedMeshVATData(
					...
					TArray <FVector4>& OutGridBoneRot)
				{
					...
					if (Profile->Anims_Vert.Num())
					{
						...
						{
							PreviewComponent->EnablePreview(true, Profile->Anims_Vert[i].SequenceRef.LoadSynchronous());
					...
					if (Profile->Anims_Bone.Num())
					{
						...
						{
							PreviewComponent->EnablePreview(true, Profile->Anims_Bone[i].SequenceRef.LoadSynchronous());
							UAnimSingleNodeInstance* SingleNodeInstance = PreviewComponent->GetSingleNodeInstance();
				...
				static void EncodeData_Vec(const TArray <FVector4>& VectorData, const float MaxValue, const bool HDR, TArray <FFloat16Color>& Data)
				{
					for (int32 i = 0; i < VectorData.Num(); i++)
					{
						...
						if (MaxDim > 0.f)
						{
							...
							{
								typedef NCsAnimation::NVertex::NTool::FUtility VertexAnimUtility;

								VectorValue.X = VertexAnimUtility::EncodeFloat(VectorValue.X, MaxDim);
								VectorValue.Y = VertexAnimUtility::EncodeFloat(VectorValue.Y, MaxDim);
								VectorValue.Z = VertexAnimUtility::EncodeFloat(VectorValue.Z, MaxDim);
								Mag = MaxDim / MaxValue;
					...
					for (int32 i = 0; i < VectorData.Num(); i++)
					{
						...
						if (false)
						{
							...
							if (MaxDim > 0.f)
							{
								...
								FVector4 Encoded = VertexAnimUtility::BitEncodeVecId_HD(WinnerValue, 1.0, BigComp);
								if(HD)
									Data[i] = FLinearColor(Encoded);
								else 
									Data[i] = FLinearColor(VertexAnimUtility::BitEncodeVecId(WinnerValue, 1.0, BigComp));
						...
						else
						{
							if (MaxDim > 0.f) 
							{
								...
								float R = FMath::Max(0.001f, VertexAnimUtility::EncodeFloat(WinnerValue.X, MaxDim)) * (Bit0 ? 1.0 : -1.0);
								//R = VertexAnimUtility::EncodeFloat(R * (Bit0 ? 1.0 : -1.0), 1.0);

								float G = FMath::Max(0.001f, VertexAnimUtility::EncodeFloat(WinnerValue.Y, MaxDim)) * (Bit1 ? 1.0 : -1.0);
								//G = VertexAnimUtility::EncodeFloat(G * (Bit1 ? 1.0 : -1.0), 1.0);

								float B = WinnerValue.Z / MaxDim;// VertexAnimUtility::EncodeFloat(WinnerValue.Z, MaxDim);
				...
				float FUtility::PackBits(const uint32& bit)
				{
					/*
					int32 f16 = bit;
					f16 += 1024;
				...
				int FUtility::UnPackBits(const float N)
				{
				...
				void FUtility::DoBakeProcess(UCsVertexAnimProfile* Profile)
				{
					...
					TSharedRef<ISkeletalMeshEditor> IEditor = SkeletalMeshEditorModule.CreateSkeletalMeshEditor(Mode, TSharedPtr<IToolkitHost>(), Profile->SkeletalMesh.LoadSynchronous());

					DoBakeProcess(Profile, IEditor->GetPersonaToolkit()->GetPreviewMeshComponent());

					GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->CloseAllEditorsForAsset(Profile->SkeletalMesh.LoadSynchronous());
				}

				void FUtility::DoBakeProcess(UCsVertexAnimProfile* Profile, UDebugSkelMeshComponent* PreviewComponent)
				{

					PreviewComponent->GlobalAnimRateScale = 0.f;
	
					//UCsVertexAnimProfile* Profile = NULL;

					TSoftClassPtr<UCsVertexAnimProfile> ProfileSoftClass(Profile);
...

					{
						...
						*/
						PackageName = ProfileSoftClass.GetLongPackageName();
						MeshName = ProfileSoftClass.GetAssetName();
					}
					...
					if (DoStaticMesh)
					{
						...
						UStaticMesh* StaticMesh = VertexAnimUtility::ConvertMeshesToStaticMesh( { PreviewComponent }, FTransform::Identity, PackageName);

						if (Profile->UVChannel_VertAnim != -1) VertexAnimUtility::VATUVsToStaticMeshLODs(StaticMesh, Profile->UVChannel_VertAnim, UVs_VertAnim);
						if (Profile->UVChannel_BoneAnim != -1) VertexAnimUtility::VATUVsToStaticMeshLODs(StaticMesh, Profile->UVChannel_BoneAnim, UVs_BoneAnim1);
						if (Profile->UVChannel_BoneAnim_Full != -1)
						{
							VertexAnimUtility::VATUVsToStaticMeshLODs(StaticMesh, Profile->UVChannel_BoneAnim_Full, UVs_BoneAnim2);
							VertexAnimUtility::VATColorsToStaticMeshLODs(StaticMesh, Colors_BoneAnim);
						}
						...
						if(Profile->Anims_Vert.Num())
						{
...

							{
								...
								Profile->NormalsTexture = SetTexture2(PreviewComponent->GetWorld(), PackagePath,
									Profile->GetName() + "_Normals", Profile->NormalsTexture.LoadSynchronous(),
...

							{
								...
								Profile->OffsetsTexture = SetTexture2(PreviewComponent->GetWorld(), PackagePath,
									Profile->GetName() + "_Offsets", Profile->OffsetsTexture.LoadSynchronous(),
									TextureWidth_Vert, TextureHeight_Vert,
									Data,
									Profile->GetMaskedFlags() | RF_Public | RF_Standalone);
						...
						if (Profile->Anims_Bone.Num())
						{
			...
			
							{
								...
								Profile->BoneRotTexture = SetTexture2(PreviewComponent->GetWorld(), PackagePath, 
									Profile->GetName() + "_BoneRot", Profile->BoneRotTexture.LoadSynchronous(),
...

							{
								...
								Profile->BonePosTexture = SetTexture2(PreviewComponent->GetWorld(), PackagePath,
									Profile->GetName() + "_BonePos", Profile->BonePosTexture.LoadSynchronous(),
									TextureWidth_Bone, TextureHeight_Bone, 
									Data,//BonePos,
									Profile->GetMaskedFlags() | RF_Public | RF_Standalone);
				...
				void FUtility::IntoIslands(const TArray<int32>& IndexBuffer, const TArray<FVector2D>& UVs, TArray<int32>& OutIslandIDs, int32& OutNumIslands)
				{
				...
				void FUtility::ClosestUVPivotAssign(
