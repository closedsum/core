...
namespace NCsSoundSpawnFrequency
{
	namespace Ref
	{
		...
		CSSOUND_API CS_ADD_TO_ENUM_MAP(Once);
		CSSOUND_API CS_ADD_TO_ENUM_MAP(Count);
		CSSOUND_API CS_ADD_TO_ENUM_MAP(TimeCount);
		CSSOUND_API CS_ADD_TO_ENUM_MAP(TimeInterval);
		CSSOUND_API CS_ADD_TO_ENUM_MAP(Infinite);
		CSSOUND_API CS_ADD_TO_ENUM_MAP_CUSTOM(ECsSoundSpawnFrequency_MAX, "MAX");
	}

	CSSOUND_API const uint8 MAX = (uint8)Type::ECsSoundSpawnFrequency_MAX;
}
	...
	namespace NSpawn
	{
		namespace NFrequency
		{
			namespace Ref
			{
				...
				CSSOUND_API CS_ADD_TO_ENUM_MAP(TimeInterval);
				CSSOUND_API CS_ADD_TO_ENUM_MAP(Infinite);
				CSSOUND_API CS_ADD_TO_ENUM_MAP_CUSTOM(EFrequency_MAX, "MAX");
			}

			CSSOUND_API const uint8 MAX = (uint8)EFrequency::EFrequency_MAX;
		}
	}
}
...
bool FCsSound_Spawn_FrequencyParams::IsValid(const FString& Context, void(*Log)(const FString&) /*=&NCsSound::FLog::Warning*/) const
{
	// Once
	if (Type == FrequencyType::Once)
	{
...
namespace NCsSound
{
	namespace NSpawn
	{
		namespace NParams
		{
			...
			bool FFrequency::IsValid(const FString& Context, void(*Log)(const FString&) /*=&NCsSound::FLog::Warning*/) const
			{
				// Once
				if (GetType() == FrequencyType::Once)
				{
...
namespace NCsSound
{
	namespace NSpawn
	{
		namespace NParams
		{
			...
			bool FParams::IsValid(const FString& Context, void(*Log)(const FString&) /*=&NCsSound::FLog::Warning*/) const
			{
				// Check Sound is Valid.
				CS_IS_VALID(Sound)
...
bool FCsSound_Spawn_Params::IsValid(const FString& Context, void(*Log)(const FString&) /*=&NCsSound::FLog::Warning*/) const
{
	// Check Sound is Valid
	CS_IS_VALID(Sound)
	CS_IS_FLOAT_GREATER_THAN(VolumeMultiplier, 0.0f)
