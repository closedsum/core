...
void UCsBTService_SetRandomLocation::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	if (NavSys)
	{
		...
		{
			UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTService_SetRandomLocation::TickNode: No NavigationData found."));
		}
	}
	else
	{
		UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTService_SetRandomLocation::TickNode: No NavigationSystem found."));
	}
}