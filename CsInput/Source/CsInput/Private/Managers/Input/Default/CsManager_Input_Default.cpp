...
namespace NCsManagerInput
{
	namespace NCached
	{
		namespace Str
		{
			CSINPUT_API CS_DEFINE_CACHED_FUNCTION_NAME_AS_STRING(UCsManager_Input_Default, SetCurrentInputActionMap);
			CSINPUT_API CS_DEFINE_CACHED_FUNCTION_NAME_AS_STRING(UCsManager_Input_Default, ClearCurrentInputActionMap);
	...
	namespace NCurrentMode
	{
		namespace NCached
		{
			namespace Str
			{
				CSINPUT_API CS_DEFINE_CACHED_FUNCTION_NAME_AS_STRING(UCsManager_Input_Default::FCurrentMode, SetValue);
				CSINPUT_API CS_DEFINE_CACHED_FUNCTION_NAME_AS_STRING(UCsManager_Input_Default::FCurrentMode, ClearValue);
			}
		}
	}
		...
		CS_CVAR_LOG_IS_SHOWING(LogInputMouseLocationEventChange))
	{
		...
		// Default__MousePositionXY__
		{
			if (OwnerAsController->GetMousePosition(MousePosition.X, MousePosition.Y))
			{
				...
				if (LogLocation)
				{
					UE_LOG(LogCsInput, Warning, TEXT("%s (%d): Time: %f."), *Context, ControllerId, Time);
					UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s. Location: (%3.3f, %3.3f)"), Action.ToChar(), EMCsInputEvent::Get().ToChar(Event), MousePosition.X, MousePosition.Y);
				}
				...
				if (Info.Location != MousePosition)
				{
					...
					{
						UE_LOG(LogCsInput, Warning, TEXT("%s (%d): Time: %f."), *Context, ControllerId, Time);
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s -> FirstMoved. Location: (%3.3f, %3.3f) -> (%3.3f, %3.3f)"), Action.ToChar(), EMCsInputEvent::Get().ToChar(Event), Location.X, Location.Y, MousePosition.X, MousePosition.Y);
					}
					// FirstMoved -> Moved
					else
					if (Event == ECsInputEvent::FirstMoved)
					{
						UE_LOG(LogCsInput, Warning, TEXT("%s (%d): Time: %f."), *Context, ControllerId, Time);
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: FirstMoved -> Moved. Location: (%3.3f, %3.3f) -> (%3.3f, %3.3f)"), Action.ToChar(), Location.X, Location.Y, MousePosition.X, MousePosition.Y);
					}
				}
				// No Change
				else
				{
					...
					{
						UE_LOG(LogCsInput, Warning, TEXT("%s (%d): Time: %f."), *Context, ControllerId, Time);
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s -> FirstStationary. Location: (%3.3f, %3.3f)"), Action.ToChar(), EMCsInputEvent::Get().ToChar(Event), MousePosition.X, MousePosition.Y);
					}
					// FirstStationary -> Stationary
					else
					if (Event == ECsInputEvent::FirstStationary)
					{
						UE_LOG(LogCsInput, Warning, TEXT("%s (%d): Time: %f."), *Context, ControllerId, Time);
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: FirstStationary -> Stationary. Location: (%3.3f, %3.3f)"), Action.ToChar(), MousePosition.X, MousePosition.Y);
					}
				}
			}
		}
		...
		for (const FECsInputAction& Action : MouseActions)
		{
			...
			if (OwnerAsController->PlayerInput->IsPressed(Key))
			{
					...
					Last_Event == ECsInputEvent::Released)
				{
					...
					{
						UE_LOG(LogCsInput, Warning, TEXT("%s (%d): Time: %f."), *Context, ControllerId, Time);
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s -> FirstPressed. Location: (%3.3f, %3.3f)"), Action.ToChar(), EMCsInputEvent::Get().ToChar(Event), MousePosition.X, MousePosition.Y);
					}
				}
				// FirstPressed -> Pressed
				else
				if (Last_Event == ECsInputEvent::FirstPressed)
				{
					...
					{
						UE_LOG(LogCsInput, Warning, TEXT("%s (%d): Time: %f."), *Context, ControllerId, Time);
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: FirstPressed -> Pressed. Location: (%3.3f, %3.3f)"), Action.ToChar(), MousePosition.X, MousePosition.Y);
					}
				}
			}
			// FirstPressed | Pressed -> FirstReleased
			...
			else
			{
					...
					Last_Event == ECsInputEvent::Pressed)
				{
					...
					{
						UE_LOG(LogCsInput, Warning, TEXT("%s (%d): Time: %f."), *Context, ControllerId, Time);
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s -> FirstReleased. Location: (%3.3f, %3.3f)"), Action.ToChar(), EMCsInputEvent::Get().ToChar(Event), MousePosition.X, MousePosition.Y);
					}
				}
				// FirstReleased -> Released
				else
				if (Last_Event == ECsInputEvent::FirstReleased)
				{
					...
					{
						UE_LOG(LogCsInput, Warning, TEXT("%s (%d): Time: %f."), *Context, ControllerId, Time);
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: FirstReleased -> Released. Location: (%3.3f, %3.3f)"), Action.ToChar(), MousePosition.X, MousePosition.Y);
					}
				}
			}
		}
			...
			for (const FECsInputAction& Action : ActionsToPrint)
			{
				...
				if (Type == ECsInputType::Action)
				{
						 ...
						 LogActionEventChange))
					{
						...
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s -> %s. Location: (%3.3f, %3.3f)"), Action.ToChar(), *LastEvent, *CurrentEvent, Location.X, Location.Y);
					}
					else
					if (LogAction)
					{
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s. Location: (%3.3f, %3.3f)"), Action.ToChar(), *CurrentEvent, Location.X, Location.Y);
					}
				}
			}
		}
	...
	if (CS_CVAR_LOG_IS_SHOWING(LogInputTouchLocation))
	{
		...
		if (Indices.Num() > CS_EMPTY)
		{
			...
			for (const int32& I : Indices)
			{
				...
				if (IsPressed)
				{
					UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Location: (%3.3f, %3.3f)"), Action.ToChar(), Touch.X, Touch.Y);
				}
			}
		}
	}
			...
			for (const int32& I : Indices)
			{
				...
				if (Info.Location != CurrentPosition)
				{
					...
					if (Event == ECsInputEvent::FirstStationary ||
						Event == ECsInputEvent::Stationary)
					{
						UE_LOG(LogCsInput, Warning, TEXT(" %s: %s -> FirstMoved. Location: (%3.3f, %3.3f)"), Action.ToChar(), EMCsInputEvent::Get().ToChar(Event), Touch.X, Touch.Y);
					}
					// FirstMoved -> Moved
					else
					if (Event == ECsInputEvent::FirstMoved)
					{
						UE_LOG(LogCsInput, Warning, TEXT(" %s: FirstMoved -> Moved. Location: (%3.3f, %3.3f)"), Action.ToChar(), Touch.X, Touch.Y);
					}
				}
				// No Change
				else
				{
					...
					if (Event == ECsInputEvent::FirstMoved ||
						Event == ECsInputEvent::Moved)
					{
						UE_LOG(LogCsInput, Warning, TEXT(" %s: %s -> FirstStationary. Location: (%3.3f, %3.3f)"), Action.ToChar(), EMCsInputEvent::Get().ToChar(Event), Touch.X, Touch.Y);
					}
					// FirstStationary -> Stationary
					else
					if (Event == ECsInputEvent::FirstStationary)
					{
						UE_LOG(LogCsInput, Warning, TEXT(" %s: FirstStationary -> Stationary. Location: (%3.3f, %3.3f)"), Action.ToChar(), Touch.X, Touch.Y);
					}
				}
			}
		}
			...
			for (const FECsInputAction& Action : ActionsToPrint)
			{
				...
				if (Type == ECsInputType::Action)
				{
						 ...
						 LogActionEventChange))
					{
						...
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s -> %s."), Action.ToChar(), *LastEvent, *CurrentEvent);
					}
					else
					if (LogAll ||
						LogAction)
					{
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s."), Action.ToChar(), *CurrentEvent);
					}
				}
				// Axis
				else
				if (Type == ECsInputType::Axis)
				{
						 ...
						 LogAxisEventChange))
					{
						...
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s -> %s. Value: %f -> %f."), Action.ToChar(), *LastEvent, *CurrentEvent, Info.Last_Value, Value);
					}
					else
					if (LogAll ||
						LogAxis)
					{
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s. Value: %f."), Action.ToChar(), *CurrentEvent, Value);
					}
				}
				// Location
				else
				if (Type == ECsInputType::Location)
				{
						 ...
						 LogLocationEventChange))
					{
						...
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s -> %s. Location: %s -> %s."), Action.ToChar(), *LastEvent, *CurrentEvent, *LastLocation, *(Location.ToString()));
					}
					else
					if (LogAll ||
						LogLocation)
					{
						UE_LOG(LogCsInput, Warning, TEXT(" Action: %s. Event: %s. Location: %s."), Action.ToChar(), *CurrentEvent, *(Location.ToString()));
					}
				}
			}
		}
...
void UCsManager_Input_Default::OnPostProcessInput_LogCurrentInputFrame()
{
	...
	if (CS_CVAR_LOG_IS_SHOWING(LogInputFrame))
	{
		...
		if (Count > CS_EMPTY)
		{
			UE_LOG(LogCsInput, Warning, TEXT("%s (%d): %d Inputs. Time: %f."), *Context, ControllerId, Count, CurrentInputFrame->Time.Time);
			...
			for (int32 I = 0; I < Count; ++I)
			{
				...
				UE_LOG(LogCsInput, Warning, TEXT(" [%d] Action: %s Event: %s."), I, Action.ToChar(), *EventAsString);
			}
		}
	}
}
		...
		if (Count > CS_EMPTY)
		{
			...
			for (FCsGameEventInfo& Info : CurrentGameEventInfos)
			{
				if (Info.IsValid(Context, nullptr))
				{
					...
					UE_LOG(LogCsInput, Warning, TEXT(" Event: %s."), Event.ToChar());
					UE_LOG(LogCsInput, Warning, TEXT(" Value: %f."), Info.Value);
					UE_LOG(LogCsInput, Warning, TEXT(" Location: %s."), *(Info.Location.ToCompactString()));
				}
			}
		}
	}
...
void UCsManager_Input_Default::SetupGameEventDefinitions()
{
	...
	for (const FECsGameEvent& Event : EMCsGameEvent::Get())
	{
		if (!InputSentenceByGameEventMap.Find(Event))
		{
			UE_LOG(LogCsInput, Warning, TEXT("UCsManager_Input_Default::SetupGameEventDefinitions (%s): No GameEventDefinition set for GameEvent: %s."), *(GetOwner()->GetName()), Event.ToChar());
		}
	}
#endif // #if WITH_EDITOR
}
			...
			for (FCsGameEventDefinition& Def : GameEventDefinitions)
			{
				...
				if (Sentence.IsCompleted())
				{
						...
						Sentence.Phrases[CS_FIRST].Words[CS_FIRST].AndInputs.Num() > CS_EMPTY)
					{
						...
						UE_LOG(LogCsInput, Warning, TEXT(" (%s, %s) -> %s."), Action.ToChar(), *InputEventAsString, Event.ToChar());
					}
				}
			}
		}
...
void UCsManager_Input_Default::FActiveMode::PrintSet(const FString& Context, const FECsInputAction& Action, const FCsInputInfo& Info, const ECsInputMode& Mode)
{
	if (CS_CVAR_LOG_IS_SHOWING(LogInputActiveMode))
	{
		...
		UE_LOG(LogCsInput, Warning, TEXT("%s: Action: %s Type: %s Event: %s Key: %s"), *Context, Action.ToChar(), EMCsInputType::Get().ToChar(Type), EMCsInputEvent::Get().ToChar(Event), *(Key.ToString()));
		UE_LOG(LogCsInput, Warning, TEXT(" Setting Mode: %s."), EMCsInputMode::Get().ToChar(Mode));
		...
		UE_LOG(LogCsInput, Warning, TEXT(" Value (%s) -> (%s)"), *(EMCsInputMode::Get().MaskToString(Previous)), *(EMCsInputMode::Get().MaskToString(New)));
	}
}
...
void UCsManager_Input_Default::FActiveMode::PrintSummary(const FString& Context)
{
	if (CS_CVAR_LOG_IS_SHOWING(LogInputActiveMode))
	{
		UE_LOG(LogCsInput, Warning, TEXT("%s: Value (%s) -> (%s)"), *Context, *(EMCsInputMode::Get().MaskToString(Last_Value)), *(EMCsInputMode::Get().MaskToString(Value)));
	}
}
...
void UCsManager_Input_Default::LoadInputProfile()
{
	...
	if (FFileHelper::LoadFileToString(DataJson, *InputProfileFilename))
	{
		...
		{
			UE_LOG(LogCsInput, Warning, TEXT("UCsManager_Input_Default::LoadInputProfile: %s is NOT Valid"), *InputProfileFilename);
		}
	}
	else
	{
		UE_LOG(LogCsInput, Warning, TEXT("UCsManager_Input_Default::LoadInputProfile: %s NOT found. Loading Default Input Profile."), *InputProfileFilename);
