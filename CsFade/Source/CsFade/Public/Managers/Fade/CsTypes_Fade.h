...
struct CSFADE_API FCsFade_ClearToBlackParams
{
...
#undef ParamsType
	...
	bool IsValid(const FString& Context, void(*Log)(const FString&) = &NCsFade::FLog::Warning) const;
	bool IsTopValidChecked(const FString& Context) const;
	bool IsTopValid(const FString& Context, void(*Log)(const FString&) = &NCsFade::FLog::Warning) const;
...
*/
USTRUCT(BlueprintType)
struct CSFADE_API FCsFade_BlackToClearParams
{
	GENERATED_USTRUCT_BODY()
...
*/
USTRUCT(BlueprintType)
struct CSFADE_API FCsFadeParams
{
	GENERATED_USTRUCT_BODY()
...
namespace NCsFade
{
	...
	public:
		...
		FORCEINLINE void Copy(const FParams& B)
		{
			CS_COPY_PROXY_TO_PROXY_AS_VALUE(B, From);
			CS_COPY_PROXY_TO_PROXY_AS_VALUE(B, To);
			CS_COPY_PROXY_TO_PROXY_AS_VALUE(B, InterpolationMethod);
			CS_COPY_PROXY_TO_PROXY_AS_VALUE(B, Easing);
			CS_COPY_PROXY_TO_PROXY_AS_VALUE(B, Curve);
			CS_COPY_PROXY_TO_PROXY_AS_VALUE(B, Time);
			CS_COPY_PROXY_TO_PROXY_AS_VALUE(B, bCollapseOnEnd);
		}
