...
void CsEthereumAccount::Parse(const FString &Str)
{
	...
	{
		UE_LOG(LogCsBlockchain, Warning, TEXT("CsEthereumAccount::Parse: Input string is NOT a Valid Account string."));
	}
}
...
void CsEthereumAccount::ParseFromFilePath(const FString &Path)
{
	...
	{
		UE_LOG(LogCsBlockchain, Warning, TEXT("CsEthereumAccount::ParseFromFilePath: Path %s does NOT Exist."), *Path);
	}
}
