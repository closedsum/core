...
NCsRoutine::FResource* FCsCoroutineSchedule::GetRoutineContainer(const FCsRoutineHandle& Handle) const
{
	...
	{
		UE_LOG(LogCsCoroutine, Warning, TEXT("FCsCoroutineSchedule::GetRoutineContainer: Handle's Index: %d is not associated with any Routine with Group: %s."), Handle.GetIndex(), Group.ToChar())
		return nullptr;
	}
