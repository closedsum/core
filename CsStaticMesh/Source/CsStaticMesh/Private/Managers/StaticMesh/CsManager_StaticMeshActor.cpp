...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CsManager_StaticMeshActor)
...
/*static*/ void UCsManager_StaticMeshActor::Init(UObject* InRoot, TSubclassOf<UCsManager_StaticMeshActor> ManagerStaticMeshActorClass, UObject* InOuter /*=nullptr*/)
{
	...
	{
		UE_LOG(LogCsStaticMesh, Warning, TEXT("UCsManager_StaticMeshActor::Init: Init has already been called."));
	}
#else
	s_bShutdown = false;
...
/*static*/ void UCsManager_StaticMeshActor::Shutdown(UObject* InRoot /*=nullptr*/)
{
	...
	if (!s_Instance)
	{
		UE_LOG(LogCsStaticMesh, Warning, TEXT("UCsManager_StaticMeshActor::Shutdown: Manager has already been shutdown."));
		return;
	}
...
void UCsManager_StaticMeshActor::SetupInternal()
{
	...
	{
		UCsStaticMeshSettings* ModuleSettings = GetMutableDefault<UCsStaticMeshSettings>();
...
void UCsManager_StaticMeshActor::InitInternalFromSettings()
{
	...
	if (Settings.PoolParams.Num() > CS_EMPTY)
	{
		...
		for (const TPair<FECsStaticMeshActor, FCsSettings_Manager_StaticMeshActor_PoolParams>& Pair : Settings.PoolParams)
		{
			...
			if (!Params.Class.Get())
			{
				UE_LOG(LogCsStaticMesh, Warning, TEXT("%s: Class @ for Type: %s is NOT already loaded in memory."), *Context, *(Params.Class.ToString()), Type.ToChar());
			}
#endif // #if !UE_BUILD_SHIPPING
...
void UCsManager_StaticMeshActor::CreatePool(const FECsStaticMeshActor& Type, const int32& Size)
{
	...
	{
		UE_LOG(LogCsStaticMesh, Warning, TEXT("UCsManager_StaticMeshActor::CreatePool: Pool for Sound: %s has already been created with Size: %d."), Type.ToChar(), PoolSize);
	}
...
{
	UE_LOG(LogCsStaticMesh, Warning, TEXT("%s"), *Str);
}
...
void UCsManager_StaticMeshActor::LogTransaction(const FString& Context, const ECsPoolTransaction& Transaction, const FCsStaticMeshActorPooled* Object)
{
	if (CS_CVAR_LOG_IS_SHOWING(LogManagerStaticMeshActorTransactions))
	{
		...
		{
			UE_LOG(LogCsStaticMesh, Warning, TEXT("%s: %s %s of Type: %s for %s attached to %s at %f."), *Context, *TransactionAsString, *ObjectName, *ClassName, *OwnerName, *ParentName, CurrentTime);
		}
		else 
		if (ObjectOwner)
		{
			UE_LOG(LogCsStaticMesh, Warning, TEXT("%s: %s %s of Type: %s for %s at %f."), *Context, *TransactionAsString, *ObjectName, *ClassName, *OwnerName, CurrentTime);
		}
		else
		if (Parent)
		{
			UE_LOG(LogCsStaticMesh, Warning, TEXT("%s: %s %s of Type: %s attached to %s at %f."), *Context, *TransactionAsString, *ObjectName, *ClassName, *ParentName, CurrentTime);
		}
		else
		{
			UE_LOG(LogCsStaticMesh, Warning, TEXT("%s: %s %s of Type: %s at %f."), *Context, *TransactionAsString, *ObjectName, *ClassName, CurrentTime);
		}
	}
}
