...
EBTNodeResult::Type UCsBTTask_LookAtAndRotateToFace::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (CsCVarLogAIBTTasks->GetInt() == CS_CVAR_SHOW_LOG)
	{
		...
		UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_LookAtAndRotateToFace::ExecuteTask (%s.%s): %s."), *(Pawn->GetName()), *(BTree->GetName()), *ResultAsString);
	}
	...
	else
	{
		...
		if (AbsDeltaYaw <= AngleDeltaForSuccess)
		{
			...
			if (CsCVarLogAIBTTasks->GetInt() == CS_CVAR_SHOW_LOG)
			{
				...
				UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_LookAtAndRotateToFace::TickTask (%s.%s): Succeeded."), *(Pawn->GetName()), *(BTree->GetName()));
			}
#endif // #if !UE_BUILD_SHIPPING
		}
		// LERP to goal Angle
	...
	if (AAIController* AIController = OwnerComp.GetAIOwner())
	{
		if (ACsAIPawn* Pawn = Cast<ACsAIPawn>(AIController->GetPawn()))
		{
			...
			if (CsCVarLogAIBTTasks->GetInt() == CS_CVAR_SHOW_LOG)
			{
				...
				UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_LookAtAndRotateToFace::AbortTask (%s.%s): Aborted."), *(Pawn->GetName()), *(BTree->GetName()));
			}
#endif // #if !UE_BUILD_SHIPPING
		}
		CleanUp(*AIController, NodeMemory);
