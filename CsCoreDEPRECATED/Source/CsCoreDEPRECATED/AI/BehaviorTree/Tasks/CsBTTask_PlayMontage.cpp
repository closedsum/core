...
EBTNodeResult::Type UCsBTTask_PlayMontage::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (!Pawn)
	{
		UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_PlayMontage::ExecuteTask (%s.%s): This Task only works with Pawns derived from ACsAIPawn."), *(BasePawn->GetName()), *(BTree->GetName()));
		return EBTNodeResult::Failed;
	}
	...
	if (bAnim && !Anim)
	{
		UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_PlayMontage::ExecuteTask (%s.%s): No Anim Set."), *(BasePawn->GetName()), *(BTree->GetName()));
		return EBTNodeResult::Failed;
	}
#endif // #if WITH_EDITORONLY_DATA
...
#endif // #if WITH_EDITORONLY_DATA
	{
		...
		if (!AnimToPlay)
		{
			UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_PlayMontage::ExecuteTask (%s.%s): Data: %s has NO Anim set for AnimType: %s"), *(BasePawn->GetName()), *(BTree->GetName()), *(Data->ShortCode.ToString()), *(AnimType.Name));
			return EBTNodeResult::Failed;
		}
	}
	...
	if (!bWaitUntilFinished)
	{
		...
		{
			UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_PlayMontage::ExecuteTask (%s.%s): Succeeded."), *(Pawn->GetName()), *(BTree->GetName()));
		}
	...
	if (CsCVarLogAIBTTasks->GetInt() == CS_CVAR_SHOW_LOG)
	{
		UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_PlayMontage::ExecuteTask (%s.%s): InProgress."), *(Pawn->GetName()), *(BTree->GetName()));
	}
#endif // #if !UE_BUILD_SHIPPING
	...
	else
	{
		...
		if (MyMemory->ElapsedTime > Time)
		{
			...
			if (CsCVarLogAIBTTasks->GetInt() == CS_CVAR_SHOW_LOG)
			{
				...
				UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_PlayMontage::TickTask (%s.%s): Succeeded."), *(Pawn->GetName()), *(BTree->GetName()));
			}
#endif // #if !UE_BUILD_SHIPPING
	...
	if (AAIController* AIController = OwnerComp.GetAIOwner())
	{
		if (ACsAIPawn* Pawn = Cast<ACsAIPawn>(AIController->GetPawn()))
		{
			...
			if (CsCVarLogAIBTTasks->GetInt() == CS_CVAR_SHOW_LOG)
			{
				...
				UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsBTTask_PlayMontage::AbortTask (%s.%s): Aborted."), *(Pawn->GetName()), *(BTree->GetName()));
			}
#endif // #if !UE_BUILD_SHIPPING
		}
	}
