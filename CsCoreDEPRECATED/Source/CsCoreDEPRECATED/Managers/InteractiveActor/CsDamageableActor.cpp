...
void ACsDamageableActor::Respawn()
{
	...
	Payload->SetName(NCsDamageableActorCached::Str::Respawn_Internal);
	Payload->SetFName(NCsDamageableActorCached::Name::Respawn_Internal);
...
PT_THREAD(ACsDamageableActor::Respawn_Internal(FCsRoutine* R))
{
	...
	const FCsTime& CurrentTime = UCsManager_Time::Get(D->GetGameInstance())->GetTime(R->GetGroup());
