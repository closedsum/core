...
FCsScopedTimerHandle::FCsScopedTimerHandle() :
	Index(INDEX_NONE),
	Handle()
{
}
...
void FCsScopedTimer::Log()
{
	...
	{
		UE_LOG(LogCsScopedTimer, Log, TEXT("%s: Time: %fms AvgTime: %fms TotalTime: %fms Ticks: %d"), **Name, (float)(1000.0 * Time), (float)(1000.0 * AvgTime), (float)(1000.0 * TotalTime), Ticks);
	}
}
...
void FCsScopedGroupTimer::Log()
{
	...
	{
		UE_LOG(LogCsScopedTimer, Log, TEXT("%s: Time: %fms AvgTime: %fms Ticks: %d"), Group.DisplayNameToChar(), (float)(1000.0 * Time), (float)(1000.0 * AvgTime), Ticks);
	}
}
