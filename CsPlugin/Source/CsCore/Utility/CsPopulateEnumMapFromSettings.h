	...
	static void FromDataTable(const FString& Context, UObject* ContextRoot, TSoftObjectPtr<UDataTable> DT_SoftObject, const FString& EnumName, void(*Log)(const FString&))
	{
		...
		if (DT)
		{
			...
			const TMap<FName, uint8*>& RowMap = DT->GetRowMap();
			{
				...
				UStrProperty* NameProperty		  = Cast<UStrProperty>(RowStruct->FindPropertyByName(NamePropertyName));
				NameProperty					  = NameProperty ? NameProperty : Cast<UStrProperty>(RowStruct->CustomFindProperty(NamePropertyName));
				UStrProperty* DisplayNameProperty = Cast<UStrProperty>(RowStruct->FindPropertyByName(DisplayNamePropertyName));
				DisplayNameProperty				  = DisplayNameProperty ? DisplayNameProperty : Cast<UStrProperty>(RowStruct->CustomFindProperty(DisplayNamePropertyName));
