...
#include "CsManager_Sense_DEPRECATED.generated.h"
...
UCLASS()
class CSCOREDEPRECATED_API ACsManager_Sense_DEPRECATED : public AActor
{
...
public:
	...
	TMap<FECsSenseActorType, TMap<uint64, FCsSenseInfo_DEPRECATED>> SenseMap;
	TMap<uint8, FCsSenseInfo_DEPRECATED*> SenseTraceRequestMap;
...
#pragma endregion Debug
	...
	virtual FCsSenseInfo_DEPRECATED* Add(AActor* Actor, const TCsSenseTeam& Team);
	FCsSenseInfo_DEPRECATED* GetInfo(AActor* Actor);

	virtual bool CanSense(AActor* Actor);
	virtual bool IsSensing(AActor* Actor);

	virtual void OnTick(const float &DeltaSeconds);
	virtual void CheckMeToActorDot(FCsSenseInfo_DEPRECATED& Info);
	virtual void TraceViewToActorBody(FCsSenseInfo_DEPRECATED& Info);
	virtual void Async_TraceViewToActorBody_Response(const uint8 &RequestId, FCsTraceResponse* Response);
	virtual void TraceViewToActorHead(FCsSenseInfo_DEPRECATED& Info);
