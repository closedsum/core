...
namespace NCsUserWidgetAnimPlayMode
{
	namespace Ref
	{
		...
		CSUICORE_API CS_ADD_TO_ENUM_MAP(Forward);
		CSUICORE_API CS_ADD_TO_ENUM_MAP(Reverse);
		CSUICORE_API CS_ADD_TO_ENUM_MAP(PingPong);
		CSUICORE_API CS_ADD_TO_ENUM_MAP_CUSTOM(ECsUserWidgetAnimPlayMode_MAX, "MAX");
	}

	CSUICORE_API const uint8 MAX = (uint8)Type::ECsUserWidgetAnimPlayMode_MAX;
}
		...
		namespace NPlay
		{
			namespace NMode
			{
				namespace Ref
				{
					...
					CSUICORE_API CS_ADD_TO_ENUM_MAP(Reverse);
					CSUICORE_API CS_ADD_TO_ENUM_MAP(PingPong);
					CSUICORE_API CS_ADD_TO_ENUM_MAP_CUSTOM(EMode_MAX, "MAX");
				}
			}
		}
	}
...
bool FCsUserWidgetAnimPlayParams::IsValid(const FString& Context, void(*Log)(const FString&) /*=&NCsUI::NCore::FLog::Warning*/) const
{
	// Check Name is Valid
...
bool FCsUserWidget_Anim_PlayParams::IsValid(const FString& Context, void(*Log)(const FString&) /*=&NCsUI::NCore::FLog::Warning*/) const
{
	// Check Name is Valid
	CS_IS_NAME_NONE(Name)
	// Check StartAtTime is Valid
...
namespace NCsUserWidget
{
	namespace NAnim
	{
		namespace NPlay
		{
			...
			bool FParams::IsValid(const FString& Context, void(*Log)(const FString&) /*=&NCsUI::NCore::FLog::Warning*/) const
			{
			// Check Name is Valid
...
bool FCsUserWidget_Render_Scale_Anim_OneShot_ByScalarParams::IsValid(const FString& Context, void(*Log)(const FString&) /*=&NCsUI::NCore::FLog::Warning*/) const
{
		...
		namespace NScale
		{
			namespace NAnim
			{
				namespace NOneShot
				{
					...
					bool FByScalarParams::IsValid(const FString& Context, void(*Log)(const FString&) /*=&NCsUI::NCore::FLog::Warning*/) const
					{
