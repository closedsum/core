...
namespace NCsString
{
	...
	bool FLibrary::Stream_GetValue(const TCHAR*& Str, bool& OutValue)
	{
		...
		{
			if (OutString == NCsString::NCached::Str::_true || OutString == NCsString::NCached::Str::One)
			{
				OutValue = true;
				return true;
			}
			if (OutString == NCsString::NCached::Str::_false || OutString == NCsString::NCached::Str::Zero)
			{
				OutValue = false;
				return true;
			}
	...
	void FLibrary::GetBitfieldOrMask(const FString& A, const FString& B, FString& OutOrMask)
	{
		if (A.Len() != B.Len())
		{
			UE_LOG(LogCsCoreLibrary, Warning, TEXT("NCsString::FLibrary::GetBitfieldOrMask: %s's Length != %s's Length (%d != %d)."), *A, *B, A.Len(), B.Len());
			return;
		}
	...
	void FLibrary::CreateUnsetBitfield(FString& Bitfield, const int32& Length)
	{
		if (Length <= CS_EMPTY)
		{
			UE_LOG(LogCsCoreLibrary, Warning, TEXT("NCsString::FLibrary::CreateUnsetBitfield: Length: %d <= 0."), Length);
			return;
		}
	...
	void FLibrary::CreateSetBitfield(FString& Bitfield, const int32& Length)
	{
		if (Length <= CS_EMPTY)
		{
			UE_LOG(LogCsCoreLibrary, Warning, TEXT("NCsString::FLibrary::CreateSetBitfield: Length: %d <= 0."), Length);
			return;
		}
	...
	int32 FLibrary::SetBitfield(FString& Bitfield, const int32& Index /*=INDEX_NONE*/)
	{
		...
		else
		{
			...
			{
				UE_LOG(LogCsCoreLibrary, Warning, TEXT("NCsString::FLibrary::SetBitfield: Index >= Bitfield.Len() (%d >= %d)."), Index, Bitfield.Len());
				return 0;
			}
	...
	int32 FLibrary::UnsetBitfield(FString& Bitfield, const int32& Index /*=INDEX_NONE*/)
	{
		...
		else
		{
			if (Index >= Bitfield.Len())
			{
				UE_LOG(LogCsCoreLibrary, Warning, TEXT("NCsString::FLibrary::UnsetBitfield: Index >= Bitfield.Len() (%d >= %d)."), Index, Bitfield.Len());
				return 0;
			}
