...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CsSettings_Input_Enhanced)
...
{
	return GetMutableDefault<UCsInputSettings>()->EnhancedInput;
}
...
void FCsSettings_Input_Enhanced::PopulateGameEventPriorityList()
{
	...
	while (!Line.IsEmpty())
	{
		...
		{
			UE_LOG(LogCsInput, Warning, TEXT("%s: Line %d: %s is NOT a Valid FECsGameEvent."), *(EMCsGameEvent::Get().GetEnumName()), Count, *Line);
			return;
		}
		Temp.Add(Event);
