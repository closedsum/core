...
void FCsManager_ScopedTimer::ClearHandle(const FCsScopedTimerHandle& Handle)
{
	if (!Handle.IsValid())
	{
		UE_LOG(LogCsScopedTimer, Warning, TEXT("FCsManager_ScopedTimer::ClearHandle: Handle is NOT Valid."));
		return;
	}

	if (Handle.GetIndex() >= Internal.GetPoolSize())
	{
		UE_LOG(LogCsScopedTimer, Warning, TEXT("FCsManager_ScopedTimer::ClearHandle: Handle's Index >= Internal.GetPoolSize() (%d >= %d)."), Handle.GetIndex(), Internal.GetPoolSize());
		return;
	}
	...
	{
		UE_LOG(LogCsScopedTimer, Warning, TEXT("FCsManager_ScopedTimer::ClearHandle: ScopedTimer with Index: %d does NOT match Handle."), Handle.Index);
		return;
	}
