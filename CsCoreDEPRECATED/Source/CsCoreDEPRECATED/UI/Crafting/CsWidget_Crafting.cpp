...
void UCsWidget_Crafting::PopulateRecipes()
{
	...
	{
		UCsData_Recipe* Data = Datas[I].GetObject<UCsData_Recipe>();
	...
	if (Count > CS_EMPTY)
	{
		UCsData_Recipe* Data = Datas[CS_FIRST].GetObject<UCsData_Recipe>();
...
void UCsWidget_Crafting::SetRecipe(const FName& InShortCode)
{
	...
	for (int32 I = 0; I < Count; ++I)
	{
		...
		const FCsData& ID					  = DataMapping->GetLoadedData(ItemDataType, ItemShortCode);
		UCsData_Item* Item					  = ID.GetObject<UCsData_Item>();
...
void UCsWidget_Crafting::IncrementCount()
{
	...
	Payload->CoroutineImpl.BindStatic(&UCsWidget_Crafting::IncrementCount_Internal);
	Payload->StartTime = UCsManager_Time::Get(GetGameInstance())->GetTime(Group);
	Payload->Owner.SetObject(this);

	Payload->SetName(ECsWidgetCraftingCached::Str::IncrementCount_Internal);
	Payload->SetFName(ECsWidgetCraftingCached::Name::IncrementCount_Internal);
...
void UCsWidget_Crafting::DecrementCount()
{
	...
	Payload->CoroutineImpl.BindStatic(&UCsWidget_Crafting::DecrementCount_Internal);
	Payload->StartTime = UCsManager_Time::Get(GetGameInstance())->GetTime(Group);
	Payload->Owner.SetObject(this);

	Payload->SetName(ECsWidgetCraftingCached::Str::DecrementCount_Internal);
	Payload->SetFName(ECsWidgetCraftingCached::Name::DecrementCount_Internal);
...
void UCsWidget_Crafting::UpdateProgress(const uint64 &PayloadId)
{
	...
	Payload->CoroutineImpl.BindStatic(&UCsWidget_Crafting::UpdateProgress_Internal);
	Payload->StartTime = UCsManager_Time::Get(GetGameInstance())->GetTime(Group);
	Payload->Owner.SetObject(this);
	Payload->SetName(ECsWidgetCraftingCached::Str::UpdateProgress_Internal);
	Payload->SetFName(ECsWidgetCraftingCached::Name::UpdateProgress_Internal);

	ACsManager_Crafting* Manager_Crafting = ACsManager_Crafting::Get(GetWorld());
	FCsCraftingPayload* CraftingPayload   = Manager_Crafting->GetPayload(PayloadId);
	UCsData_Recipe* Recipe				  = CraftingPayload->GetRecipe();
