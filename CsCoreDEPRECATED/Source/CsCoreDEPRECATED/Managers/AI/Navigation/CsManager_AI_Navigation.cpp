...
/*static*/ UCsManager_AI_Navigation* UCsManager_AI_Navigation::Get()
{
	...
	if (!s_Instance)
	{
		UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsManager_AI_Navigation::Get: Manager must be attached and registered on a game object in order to call Get()."));
		return nullptr;
	}
...
/*static*/ void UCsManager_AI_Navigation::Init(UCsManager_AI_Navigation* Manager)
{
	...
	if (s_Instance)
	{
		UE_LOG(LogCsCoreDEPRECATED, Warning, TEXT("UCsManager_AI_Navigation::Init: This is being called before the previous instance of the manager has been Shutdown."));
	}
	s_Instance = Manager;
}
