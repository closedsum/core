...
struct CSUICORE_API FCsUserWidget : public FCsTableRowBase_Data
{
...
public:
	...
	FORCEINLINE UClass* GetSafeClass(const FString& Context, void(*Log)(const FString&) = &NCsUI::NCore::FLog::Warning) const
	{
	...
	FORCEINLINE UClass* GetSafeClass(const FString& Context, bool& OutSuccess, void(*Log)(const FString&) = &NCsUI::NCore::FLog::Warning) const
	{
		OutSuccess = false;
	...
	FORCEINLINE bool IsValid(const FString& Context, void(*Log)(const FString&) = &NCsUI::NCore::FLog::Warning) const
	{
		if (!Widget.ToSoftObjectPath().IsValid())
		{
			if (Log)
