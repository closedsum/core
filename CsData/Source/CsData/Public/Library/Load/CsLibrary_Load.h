...
namespace NCsLoad
{
	...
	#pragma region
		...
		static UClass* LoadAssetClass(const FString& AssetPath)
		{
			...
			//UE_LOG(LogCs, Warning, TEXT("CsLoadLibrary::GetAssetClass: Failed to find Asset at: %s. It is possible it was deleted or moved."), *AssetPath);
			return nullptr;
		}
		...
		static void LoadTSoftClassPtr(const FString& MemberName, TSoftClassPtr<T>& SoftClassPtr, T*& Internal, const FString& AssetErrorMessageType)
		{
			...
			else
			{
				...
				else
				{
					...
					UE_LOG(LogLoad, Warning, TEXT("CsLoadLibrary::LoadTSoftClassPtr (%s): Failed to load: %s at: %s."), *MemberName, *AssetErrorMessageType, *AssetDescription);
		...
		static void LoadTSoftClassPtr(const FString& MemberName, TSoftClassPtr<T>*& SoftClassPtr, T*& Internal, const FString& AssetErrorMessageType)
		{
			...
			if (Internal &&
				...
				Internal == SoftClassPtr->Get()->template GetDefaultObject<T>())
			{
				//UE_LOG(LogLoad, Warning, TEXT("CsLoadLibrary::LoadTSoftClassPtr (%s): Possibly trying to load: %s and it is already loaded."), *MemberName, *AssetErrorMessageType);
				return;
			}
		...
		static void LoadTSoftClassPtr(TSoftClassPtr<T> &AssetSubclassOf, T* &OutAsset, const FString &AssetErrorMessageType)
		{
			...
			else
			{
				...
				else
				{
					...
					UE_LOG(LogLoad, Warning, TEXT("CsLoadLibrary::LoadTSoftClassPtr (%s): Failed to load %s at %s"), *AssetErrorMessageType, *AssetDescription);
				}
			}
		}
...
using CsLoadLibrary = NCsLoad::FLibrary;