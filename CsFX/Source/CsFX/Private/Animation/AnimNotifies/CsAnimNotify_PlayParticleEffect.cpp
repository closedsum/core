...
void UCsAnimNotify_PlayParticleEffect::Notify(class USkeletalMeshComponent* MeshComp, class UAnimSequenceBase* Animation)
{
	if (!FX.Particle)
	{
		UE_LOG(LogCsFX, Warning, TEXT("UCsAnimNotify_PlayParticleEffect::Notify (%s): No Particle set on Notify for Anim: %s"), *(MeshComp->GetSkeletalMeshAsset()->GetName()), *(Animation->GetName()));
		return;
	}
