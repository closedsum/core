...
namespace NCsLoad
{
	...
	void FLibrary::GetDataTables(const void* StructValue, UStruct* const& Struct, TMap<FName, UDataTable*>& OutDataTableMap)
	{
	...
	void FLibrary::GetDataTables(ULevel* Level, TMap<FName, UDataTable*>& OutDataTableMap)
	{
	...
	void FLibrary::GetSoftObjectPaths(const void* StructValue, UStruct* const& Struct, TArray<FSoftObjectPath>& OutObjectPaths)
	{
		for (TPropertyValueIterator<FProperty> It(Struct, StructValue); It; ++It)
		{
			const FProperty* Property = It.Key();
	...
	void FLibrary::GetSoftObjectPaths(const void* StructValue, UStruct* const& Struct, TMap<FName, FSoftObjectPath>& OutObjectPathMap)
	{
		// Iterate through Properties
		for (TPropertyValueIterator<FProperty> It(Struct, StructValue); It; ++It)
		{
	...
	void FLibrary::GetSoftObjectPaths(ULevel* Level, TMap<FName, FSoftObjectPath>& OutObjectPathMap)
	{
		for (AActor* Actor : Level->Actors)
		{
			if (!Actor)
	...
	bool FLibrary::CanLoad(void* StructValue, UStruct* const& Struct, const FString& MemberName, const int32& LoadFlags, const int32& LoadCodes)
	{
		return true;
	...
	void FLibrary::LoadSoftClassProperty(FSoftClassProperty* SoftClassProperty, void* StructValue, UStruct* const& Struct, const FString& MemberName, const int32& LoadFlags, const int32& LoadCodes)
	{
	...
	void FLibrary::LoadArraySoftClassProperty(FArrayProperty* ArrayProperty, void* StructValue, UStruct* const& Struct, const FString& MemberName, const int32& LoadFlags, const int32& LoadCodes)
	{
		if (!CanLoad(StructValue, Struct, MemberName, LoadFlags, LoadCodes))
			return;
	...
	void FLibrary::LoadSoftObjectProperty(FSoftObjectProperty* SoftObjectProperty, void* StructValue, UStruct* const& Struct, const FString& MemberName, const int32& LoadFlags, const int32& LoadCodes)
	{
			...
			if (FObjectProperty* InternalObjectProperty = FindFProperty<FObjectProperty>(Struct, *InternalMemberName))
			{
					...
					InternalObjectProperty->PropertyClass->IsChildOf(UBlueprintGeneratedClass::StaticClass()))
				{
					if (TObjectPtr<UObject>* Internal = InternalObjectProperty->GetPropertyValuePtr_InContainer(StructValue))
					{
						...
						checkf(O, TEXT("CsLoadLibrary::LoadSoftObjectProperty: Failed to load Object at Path: %s."), *Path);
	...
	void FLibrary::LoadArraySoftObjectProperty(FArrayProperty* ArrayProperty, void* StructValue, UStruct* const& Struct, const FString& MemberName, const int32& LoadFlags, const int32& LoadCodes)
	{
		if (!CanLoad(StructValue, Struct, MemberName, LoadFlags, LoadCodes))
			return;
					...
					for (int32 I = 0; I < Count; ++I)
					{
						...
						*InternalPtr = nullptr;
						...
						if (Ptr->ToSoftObjectPath().IsValid())
						{
							...
							checkf(O, TEXT("CsLoadLibrary::LoadArraySoftObjectProperty: Failed to load Object at Path: %s."), *Path);

							UBlueprintGeneratedClass* BpGC = Cast<UBlueprintGeneratedClass>(O);

							checkf(BpGC, TEXT("CsLoadLibrary::LoadArraySoftObjectProperty: Failed to cast Object: %s to UBlueprintGeneratedClass."), *(O->GetName()));

							//checkf(BpGC->ClassGeneratedBy, TEXT("CsLoadLibrary::LoadArraySoftObjectProperty: ClassGeneratedBy is NULL for Object: %s."), *(O->GetName()));

							UBlueprintCore* BpC = nullptr;//Cast<UBlueprintCore>(BpGC->ClassGeneratedBy);

							//checkf(BpC, TEXT("CsLoadLibrary::LoadArraySoftObjectProperty: Failed to cast Class: %s to UBlueprintCore."), *(BpGC->ClassGeneratedBy->GetName()));

							checkf(BpC->GeneratedClass, TEXT("CsLoadLibrary::LoadArraySoftObjectProperty: Failed to get GeneratedClass from Class: %s."), *(BpC->GetName()));
	...
	void FLibrary::LoadClassProperty(FClassProperty* ClassProperty, void* StructValue, UStruct* const& Struct, const int32& LoadFlags, const int32& LoadCodes)
	{
	...
	void FLibrary::LoadArrayClassProperty(FArrayProperty* ArrayProperty, void* StructValue, UStruct* const& Struct, const int32& LoadFlags, const int32& LoadCodes)
	{
		FScriptArrayHelper_InContainer Helper(ArrayProperty, StructValue);
	...
	void FLibrary::LoadArrayObjectProperty(FArrayProperty* ArrayProperty, void* StructValue, const int32& LoadFlags, const int32& LoadCodes)
	{
		FScriptArrayHelper_InContainer Helper(ArrayProperty, StructValue);
	...
	void FLibrary::LoadArrayStructProperty(FArrayProperty* ArrayProperty, void* StructValue, const int32& LoadFlags, const int32& LoadCodes)
	{
		FScriptArrayHelper_InContainer Helper(ArrayProperty, StructValue);
	...
	void FLibrary::LoadStruct(void* StructValue, UStruct* const& Struct, const int32& LoadFlags, const int32& LoadCodes)
	{
	...
	void FLibrary::LoadDataTable(UDataTable* DataTable, const int32& LoadFlags, const int32& LoadCodes)
	{
	...
	void FLibrary::LoadDataTableRowChecked(const FString& Context, UDataTable* DataTable, const FName& RowName, const int32& LoadFlags, const int32& LoadCodes)
	{
	...
	bool FLibrary::SafeLoadDataTableRow(const FString& Context, UDataTable* DataTable, const FName& RowName, const int32& LoadFlags, const int32& LoadCodes, void(*Log)(const FString&) /*=&FCsLog::Warning*/)
	{
	...
	UObject* FLibrary::LoadSoftClassPtrChecked(const FString& Context, const TSoftClassPtr<UObject>& SoftClass, const int32& LoadFlags, const int32& LoadCodes)
	{
	...
	void FLibrary::UnloadSoftClassProperty(FSoftClassProperty* SoftClassProperty, void* StructValue, UStruct* const& Struct, const FString& MemberName, const int32& UnloadCodes)
	{
		if (FSoftObjectPtr* Member = SoftClassProperty->GetPropertyValuePtr_InContainer(StructValue))
		{
			if (!Member->ToSoftObjectPath().IsValid())
	...
	void FLibrary::UnloadArraySoftClassProperty(FArrayProperty* ArrayProperty, void* StructValue, UStruct* const& Struct, const FString& MemberName, const int32& UnloadCodes)
	{
		// Check if an "Internal" member exists (i.e. MemberName + _Internal)
	...
	void FLibrary::UnloadSoftObjectProperty(FSoftObjectProperty* SoftObjectProperty, void* StructValue, UStruct* const& Struct, const FString& MemberName, const int32& UnloadCodes)
	{
		if (FSoftObjectPtr* Member = SoftObjectProperty->GetPropertyValuePtr_InContainer(StructValue))
		{
			if (!Member->ToSoftObjectPath().IsValid())
	...
	void FLibrary::UnloadArraySoftObjectProperty(FArrayProperty* ArrayProperty, void* StructValue, UStruct* const& Struct, const FString& MemberName, const int32& UnloadCodes)
	{
		// Check if an "Internal" member exists (i.e. MemberName + _Internal)
	...
	void FLibrary::UnloadArrayObjectProperty(FArrayProperty* ArrayProperty, void* StructValue, const int32& UnloadCodes)
	{
		FScriptArrayHelper_InContainer Helper(ArrayProperty, StructValue);
	...
	void FLibrary::UnloadArrayStructProperty(FArrayProperty* ArrayProperty, void* StructValue, const int32& UnloadCodes)
	{
		FScriptArrayHelper_InContainer Helper(ArrayProperty, StructValue);
	...
	void FLibrary::UnloadStruct(void* StructValue, UStruct* const& Struct, const int32& UnloadCodes)
	{
		for (TFieldIterator<FProperty> It(Struct); It; ++It)
		{
			FProperty* Property = CastField<FProperty>(*It);
	...
	void FLibrary::UnloadStruct(void* StructValue, UStruct* const& Struct)
	{
	...
	void FLibrary::UnloadDataTable(UDataTable* DataTable, const int32& UnloadCodes)
	{
		const UScriptStruct* ScriptStruct = DataTable->GetRowStruct();
		UScriptStruct* Temp				  = const_cast<UScriptStruct*>(ScriptStruct);
		UStruct* const Struct			  = Temp;
	...
	UObject* FLibrary::GetDefaultObjectChecked(const FString& Context, const TSubclassOf<UObject>& SubclassOf)
	{
