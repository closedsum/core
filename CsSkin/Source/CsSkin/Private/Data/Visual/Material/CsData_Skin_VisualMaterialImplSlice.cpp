			...
			namespace NMaterial
			{
				...
				/*static*/ FImplSlice* FImplSlice::AddSafeSlice(const FString& Context, FCsInterfaceMap* InterfaceMap, ICsDeconstructInterfaceSliceMap* DeconstructInterfaceSliceMap, UObject* Object, void(*Log)(const FString&) /*=&NCsSkin::FLog::Warning*/)
				{
					...
					if (!Success)
					{
						if (Log)
						{
							Log(FString::Printf(TEXT("%s: Failed to find any properties from %s for interface: FCsData_Skin_VisualMaterialImplSlice (NCsSkin::NData::NVisual::NMaterial::IMaterial)."), *(CsObjectLibrary::PrintObjectAndClass(Object))));
